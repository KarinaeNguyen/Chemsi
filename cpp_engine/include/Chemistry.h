...
public:
    ...
    ReactionResult react(
        double dt,
        double T_K,
        // When > 0, provides an ignition/active-flame kinetics assist by flooring the
        // Arrhenius temperature used for the reaction-rate calculation (NOT the bulk
        // thermal state). This allows post-ignition combustion to start deterministically
        // without requiring the reactor's bulk temperature to already be high.
